...
function App() {
    ...
    const api = (v) =>
        `https://api.openweathermap.org/data/2.5/weather?q=${v}&appid=9f5b84354ce27b6a2e460b89759bfca7&units=metric`;
    ...
    async function getInitialLocation() {
        setIsLoading(true);
        try {
            const response = await fetch(api("montreal"), myInit);
            if (!response.ok) {
                throw new Error("bad network request");
            }
            const data = await response.json();
    ...
    return ( <
            ...
            div className = "App" >
            <
            Header errorLoading = { errorLoading }
            ...
            isLoading ? ( <
                ...
                div >
                <
                LocationInfo degreeFormat = { degreeFormat }
                location = { location }
                /> <
                SwitchButton toggleFormat = {
                    () => toggleFormat()
                }
